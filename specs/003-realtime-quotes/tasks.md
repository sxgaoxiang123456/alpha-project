...
- [ ] **T1 [BE]** 创建 HistoricalQuote 数据模型：`app/models/historical_quote.py`（stock_code, date, open, close, high, low, volume, turnover，含日期+代码复合索引）
  - [FR-007] [依赖: F1 基础设施就绪] [出参验证: `HistoricalQuote.__table__.create()` 成功，表含 8 字段 + 复合索引]

- [ ] **T2 [BE]** 创建 Quote Pydantic schemas：`app/schemas/quote.py`（Quote, MarketIndex, HistoricalQuote 请求/响应模型）
...
- [ ] **T3 [BE] [P]** 实现数据清洗服务：`app/services/data_cleaner.py`（异常值检测：价格为负/0、涨跌幅超阈值、停牌识别；返回标准化 Quote）
  - [FR-005, FR-008] [依赖: T2] [出参验证: 单元测试 — 价格为-10→标记异常；涨跌幅+25%（非科创）→标记异常；停牌股票→status="suspended"]

- [ ] **T4 [BE] [P]** 实现大盘指数服务：`app/services/market_index.py`（固定 3 个指数获取 + 缓存 + 返回 MarketIndex）
  - [FR-003] [依赖: T2] [出参验证: 单元测试 — mock facade 返回 3 个指数数据，清洗后返回标准化 MarketIndex 列表]

- [ ] **T5 [BE] [P]** 实现行情服务核心逻辑：`app/services/quote_service.py`（读取 WatchlistItem → 批量调用 facade → 清洗 → 返回 Quote 列表）
...
- [ ] **T8 [BE]** 实现行情定时任务：`app/core/quote_scheduler.py`（APScheduler 任务：交易日判断 → 每 3 分钟触发 quote_service 刷新 → 大盘指数刷新）
  - [FR-002, A-004, A-008] [依赖: T6, T7] [出参验证: 单元测试 — mock 交易日 → 任务执行刷新；mock 非交易日 → 任务跳过]

- [ ] **T9 [BE]** 实现行情查询路由：`app/routers/quotes.py`（GET /quotes 返回自选股行情，优先读缓存；GET /quotes/market 返回大盘指数）
...
- [ ] **T10 [BE]** 更新配置与入口：更新 `app/config.py`（新增行情刷新周期、缓存过期时间、交易日历配置）+ 更新 `app/main.py`（注册 quote_scheduler APScheduler 任务）
...
- [ ] **T11 [INT] [P]** 单元测试 — 数据清洗：`tests/unit/test_data_cleaner.py`（异常值规则、停牌识别、科创板阈值）
  - [FR-005, FR-008] [依赖: T3] [出参验证: pytest 全部通过]

- [ ] **T12 [INT] [P]** 单元测试 — 行情服务：`tests/unit/test_quote_service.py` + `tests/unit/test_market_index.py`（mock facade、缓存集成、落盘验证）
  - [FR-001~FR-007] [依赖: T5, T6, T7] [出参验证: pytest 全部通过]

- [ ] **T13 [INT] [P]** 单元测试 — 定时任务：`tests/unit/test_quote_scheduler.py`（交易日判断、调度逻辑、非交易时段跳过）
  - [FR-002, A-008] [依赖: T8] [出参验证: pytest 全部通过（使用 freezegun 冻结时间）]

- [ ] **T14 [INT]** 集成测试 — 主动查询 API：`tests/integration/test_quotes_api.py`（GET /quotes、GET /quotes/market、缓存命中/未命中）
  - [FR-001, FR-003, SC-001, SC-002] [依赖: T9] [出参验证: pytest 全部通过，首屏加载 < 3 秒]

- [ ] **T15 [INT]** 集成测试 — 定时刷新与落盘：`tests/integration/test_quote_scheduler.py`（定时触发 → 数据获取 → 清洗 → 缓存更新 → 历史落盘）
