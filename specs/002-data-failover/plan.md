...
```text
# 复用 F1 基础设施（不修改，仅依赖）
backend/app/config.py             # 复用 — 新增数据源配置项
backend/app/database.py           # 复用 — 新增 cache_entries 表创建
backend/app/main.py               # 复用 — 注册 APScheduler 健康检查任务
backend/app/dependencies.py       # 复用 — 注入 DB session
backend/app/schemas/              # 复用目录结构

# 本 feature 新建模块
backend/app/core/                 # 新增目录：核心基础设施（非业务服务）
│   ├── __init__.py
...
│
backend/app/routers/              # 本 feature 无新增路由（纯服务层）
│
# 测试（新增）
backend/tests/
