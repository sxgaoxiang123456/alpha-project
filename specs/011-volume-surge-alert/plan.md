...
F9 成交量异动检测 **不依赖 LLM**，也不引入 Agent 框架。但在 v1.1 三个 feature 的落地顺序上，建议按 **009（F7 AI 简报）→ 010（F8 自然语言设预警）→ 011（F9 成交量异动检测）** 推进：

- **009 优先**：建立 `BriefingLLMClient`、`PromptLoader` 和 `backend/.env` 的 `DEEPSEEK_API_KEY` 配置，为 010 的 LLM 兜底解析提供复用基础。
- **010 次之**：复用 009 的 LLM 基础设施，完成自然语言设预警；同时其规则解析器不会影响 011 的检测逻辑。
- **011 最后**：纯规则计算，依赖 MVP 的历史行情、自选股和推送模块；可与 009/010 并行设计，但建议在 009/010 的 LLM 基础设施稳定后再进入编码，避免并行改动 `quote_scheduler.py` 和 `push_service.py` 产生冲突。

本 feature 的准备工作只涉及：确认 MVP 历史行情/自选股服务可用、添加 `volume_surge_events` 数据模型、注册 15:15 定时任务。
