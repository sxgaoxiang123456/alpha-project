...
- [ ] **T0a [BE]** 创建 AppSetting 数据模型：`backend/models/app_setting.py`（key, value, category, is_encrypted, updated_at，含 category 索引）
  - [依赖: F1 基础设施就绪] [出参验证: `AppSetting.__table__.create()` 成功，表含 5 字段 + category 索引]

- [ ] **T0b [BE]** 创建 Setting Pydantic schemas：`backend/schemas/settings.py`（SettingRequest, SettingResponse, SettingCategory）
  - [依赖: T0a] [出参验证: 无效 category 触发 pydantic.ValidationError]

- [ ] **T0c [BE] [P]** 创建 SettingsService：`backend/services/settings_service.py`（`get_setting(key)`, `set_setting(key, value, encrypt=False)`, `get_all_by_category(category)`，敏感字段使用 Fernet 加密）
...
- [ ] **T1 [BE]** 创建 Dashboard Pydantic schemas：`backend/schemas/dashboard.py`（DashboardViewResponse, MarketSnapshot, StockCardData, BriefingData, AlertSummary, PushHistoryItem, ChannelStatusItem）
  - [FR-001~FR-010] [依赖: F1/F2/F3/F4 基础设施就绪, T0b] [出参验证: 无效字段触发 pydantic.ValidationError]

- [ ] **T2 [BE]** 创建 DashboardService 接口骨架：`backend/services/dashboard_service.py`（类定义 + 聚合方法签名 + 上游服务依赖注入接口）
...
- [ ] **T3 [BE]** 实现 DashboardService 数据聚合：`backend/services/dashboard_service.py`（asyncio.gather 并行调用 5 个上游服务 → 统一响应模型 → 单个服务超时降级）
  - [FR-001~FR-005, FR-009, FR-010] [依赖: T2, F1/F2/F3/F4 服务就绪] [出参验证: 单元测试 — mock 5 个上游服务 → 验证并行调用、超时降级、聚合结果完整]

- [ ] **T4 [BE]** 实现 Dashboard 首页路由：`backend/routers/dashboard.py`（GET / → 聚合服务 → 渲染 dashboard.html；GET /market_data → 返回行情 Partial HTML）
...
- [ ] **T5 [FE] [P]** 创建基础布局模板：`frontend/templates/base.html`（HTML5 骨架 + Tailwind CDN + Google Fonts + Material Symbols）+ `frontend/templates/dashboard.html`（首页主布局，12 列网格，含各组件插槽）
...
- [ ] **T6 [FE] [P]** 创建 SideNavBar 组件：`frontend/templates/components/side_nav.html`（Logo + 导航菜单 + AI 简报按钮）
...
- [ ] **T7 [FE] [P]** 创建 TopNavBar + CommandBar 组件：`frontend/templates/components/top_nav.html`（顶部栏 + 搜索框 + 通知 + 用户头像）
...
- [ ] **T8 [FE] [P]** 创建 MarketIndexCard 组件：`frontend/templates/components/market_index.html`（上证/深证/创业 → 点位 + 涨跌幅 + 涨跌额）
...
- [ ] **T9 [FE] [P]** 创建 WatchlistSnapshot 组件：`frontend/templates/components/watchlist_snapshot.html`（自选股快照表格：代码/名称/价格/涨跌幅/趋势方向）
...
- [ ] **T10 [FE] [P]** 创建 AI Briefing Card 组件：`frontend/templates/components/briefing_card.html`（每日 AI 简报：insight 列表 + 查看完整按钮）
...
- [ ] **T11 [FE] [P]** 创建 AlertBanner 组件：`frontend/templates/components/alert_banner.html`（今日触警股票列表 + 触发条件 + 预警级别图标，可折叠）
...
- [ ] **T12 [FE] [P]** 创建 PushHistory 组件：`frontend/templates/components/push_history.html`（消息类型/标题/时间/通道/状态列表，最近 100 条限制）
...
- [ ] **T13 [FE] [P]** 创建 ChannelStatus 组件：`frontend/templates/components/channel_status.html`（飞书/Telegram 状态 + 限流提示 + 颜色标记：绿/黄/红）
...
- [ ] **T14 [FE] [P]** 创建 QuickActions + Onboarding 组件：`frontend/templates/components/quick_actions.html`（4 个快捷入口按钮：添加股票/设置预警/市场复盘/系统设置）+ `frontend/templates/components/onboarding.html`（空自选股引导卡片）
...
- [ ] **T15 [FE] [P]** 实现响应式样式和刷新逻辑：`frontend/static/css/dashboard.css`（移动端单列/PC 端多列布局 + Tailwind 响应式类补充）+ `frontend/static/js/dashboard.js`（60 秒 fetch 轮询 + DOM innerHTML 替换 + 数据源恢复检测 + 暂停/恢复控制）
...
- [ ] **T16 [FE] [P]** 创建设置页模板：`frontend/templates/settings.html`（按 category 分组表单：lark / telegram / datasource / preference，敏感字段 `<input type="password">`）
...
- [ ] **T17 [BE]** 实现设置页路由：`backend/routers/settings.py`（GET /settings → 渲染设置表单；POST /settings → 接收表单数据 → 调用 `settings_service.set_setting()` 持久化；按 category 分组展示配置项）
  - [依赖: T0c, T16] [出参验证: 集成测试 — 提交飞书配置 → 验证数据库写入；读取配置 → 验证敏感字段解密正确]

- [ ] **T18 [BE]** 更新路由注册：更新 `backend/main.py`（注册 dashboard 路由、settings 路由、静态文件挂载）
...
- [ ] **T19 [INT] [P]** 单元测试 — DashboardService：`backend/test/unit/test_dashboard_service.py`（mock 5 个上游服务 → 验证并行聚合、超时降级、空数据降级）
  - [FR-001~FR-005] [依赖: T3] [出参验证: pytest 全部通过]

- [ ] **T20 [INT] [P]** 单元测试 — SettingsService：`backend/test/unit/test_settings_service.py`（写入/读取普通配置、加密配置 → 验证明文/密文存储正确；mock 无加密密钥 → 验证报错）
  - [依赖: T0c] [出参验证: pytest 全部通过]

- [ ] **T21 [INT] [P]** 集成测试 — 页面渲染：`backend/test/integration/test_dashboard.py`（GET / → 验证完整页面含所有模块；GET /market_data → 验证 Partial HTML；模拟数据源异常 → 验证降级提示）
  - [FR-001~FR-010] [依赖: T4, T5~T15] [出参验证: pytest 全部通过]

- [ ] **T22 [INT]** 响应式测试 — 多 viewport：`frontend/__test__/test_responsive.py`（模拟 375px/768px/1200px → 验证布局切换、核心信息完整展示）
