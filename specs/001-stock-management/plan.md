...
| R-PLAN-07 | **并发添加突破 100 只上限**（C1 补测发现） | **高** | **中** | **当前**：`threading.Lock()` 仅对单进程生效，已覆盖 001 全部并发测试。**发布门**：单进程 Docker 部署通过；多进程/多实例需升级为分布式锁（Redis `SET NX` 或 PostgreSQL advisory lock），见 DD-001 批注 |
