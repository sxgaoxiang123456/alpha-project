...
- [X] **T2 [BE]** 创建配置与数据库层：`backend/app/config.py`（环境变量加载）+ `backend/app/database.py`（SQLAlchemy 引擎/SessionLocal/建表）+ `backend/app/main.py`（FastAPI 入口）
  - [架构设计 DD-001] [依赖: T1] [出参验证: `uvicorn backend.app.main:app` 启动无报错，访问 `/docs` 显示 Swagger UI]

- [X] **T3 [BE] [P]** 创建 Stock 数据模型：`backend/app/models/stock.py`
  - [FR-002] [依赖: T2] [出参验证: `Stock.__table__.create()` 成功，表含 code/name/market/sector/status 字段]

- [X] **T4 [BE] [P]** 创建 Group 数据模型：`backend/app/models/group.py`，含默认分组"默认分组"自动初始化逻辑
  - [FR-004] [依赖: T2] [出参验证: 启动时自动创建 id=1 的默认分组记录]

- [X] **T5 [BE]** 创建 WatchlistItem 数据模型：`backend/app/models/watchlist.py`，含与 Stock/Group 的外键关联 + stock_code 唯一约束
  - [FR-003, FR-004, FR-008] [依赖: T3, T4] [出参验证: 插入重复 stock_code 触发 IntegrityError]

- [X] **T6 [BE] [P]** 创建 Pydantic schemas：`backend/app/schemas/stock.py` + `backend/app/schemas/watchlist.py` + `backend/app/schemas/group.py`
...
- [X] **T7 [BE] [P]** 实现股票搜索服务：`backend/app/services/stock_search.py`（AkShare 查询 + BaoStock 备用降级 + 代码格式校验）
  - [FR-001, FR-002, FR-010] [依赖: T6] [出参验证: 单元测试 — 输入"600519"返回贵州茅台对象；输入"999999"返回 None；输入"60051A"返回格式错误]

- [X] **T8 [BE]** 实现 watchlist 路由（添加/搜索/列表）：`backend/app/routers/watchlist.py`（POST /watchlist, GET /watchlist/search, GET /watchlist）
...
- [X] **T9 [BE] [P]** 实现 CSV 导入服务：`backend/app/services/csv_import.py`（解析 + 逐行校验：格式/代码有效性/去重/上限 + 部分成功处理）
  - [FR-005, FR-006] [依赖: T6] [出参验证: 单元测试 — 20行有效CSV返回成功数=20；含2行错误返回成功18+失败明细；101行CSV返回超限错误]

- [X] **T10 [BE] [P]** 实现 CSV 导出服务：`backend/app/services/csv_export.py`（查询当前自选股 + 生成 CSV 字节流）
  - [FR-012] [依赖: T6] [出参验证: 单元测试 — 导出 CSV 包含 code/name/group/cost_price/shares 列，与上传格式一致]

- [X] **T11 [BE]** 实现导入导出路由：`backend/app/routers/import_export.py`（POST /watchlist/import 文件上传, GET /watchlist/export 下载）
...
- [X] **T12 [BE] [P]** 实现分组业务服务：`backend/app/services/group_service.py`（删除分组时：组内股票移入默认分组 / 一并删除）
  - [FR-011] [依赖: T6] [出参验证: 单元测试 — 删除含3只股票的分组，选择"移入默认"则默认分组+3；选择"一并删除"则股票全部移除]

- [X] **T13 [BE]** 实现 groups 路由：`backend/app/routers/groups.py`（POST /groups 创建, GET /groups 列表, PUT /groups/{id} 重命名, DELETE /groups/{id} 删除）
...
- [X] **T14 [BE]** 扩展 watchlist 路由（编辑/删除）：`backend/app/routers/watchlist.py` 追加 PUT /watchlist/{code}（编辑成本/股数）、DELETE /watchlist/{code}（单删）、POST /watchlist/batch-delete（批量删）
...
- [X] **T15 [FE] [P]** 创建基础布局模板：`frontend/src/templates/base.html`（HTML5 骨架 + Tailwind CDN + Google Fonts + Material Symbols）
...
- [X] **T16 [FE] [P]** 创建 SideNavBar 组件：`frontend/src/templates/components/side_nav.html`（导航菜单 + AI 简报按钮）
...
- [X] **T17 [FE] [P]** 创建 TopNavBar + CommandBar 组件：`frontend/src/templates/components/top_nav.html`（顶部栏 + 搜索框 + 通知 + 用户头像）
...
- [X] **T18 [FE] [P]** 创建 FilterSidebar 组件：`frontend/src/templates/components/filter_sidebar.html`（分组列表 + 计数 + 新建分组按钮）
...
- [X] **T19 [FE] [P]** 创建 StockDataTable 组件：`frontend/src/templates/components/stock_table.html` + `stock_table_row.html`（表头控制 + 数据行 + 迷你趋势图 + 操作按钮）
...
- [X] **T20 [FE] [P]** 创建辅助状态组件：`frontend/src/templates/components/empty_state.html` + `alert_badge.html` + `metric_tag.html`
...
- [X] **T21 [FE]** 创建自选股列表主页面：`frontend/src/templates/watchlist/list.html`（组装 SideNavBar + TopNavBar + FilterSidebar + StockDataTable + EmptyState）
...
- [X] **T22 [FE] [P]** 创建添加/编辑弹窗组件：`frontend/src/templates/watchlist/add_modal.html` + `edit_modal.html`（CommandBar 搜索下拉 + 分组选择 + 成本/股数编辑）
...
- [X] **T23 [FE] [P]** 创建分组管理页面：`frontend/src/templates/groups/manage.html` + 创建分组管理 CSS/JS：`frontend/public/css/watchlist.css` + `frontend/public/js/watchlist.js`
...
- [X] **T24 [INT] [P]** 单元测试套件：`backend/tests/unit/test_models.py` + `backend/tests/unit/test_schemas.py` + `backend/tests/unit/test_stock_search.py` + `backend/tests/unit/test_csv_import.py` + `backend/tests/unit/test_csv_export.py` + `backend/tests/unit/test_group_service.py` + `backend/tests/unit/test_config_database_main.py`
  - [FR-001~FR-012] [依赖: T3~T12] [出参验证: `pytest backend/tests/unit/` 全部通过（≥ 30 个测试用例）]

- [X] **T25 [INT] [P]** 集成测试套件：`backend/tests/integration/test_watchlist_api.py` + `backend/tests/integration/test_watchlist_edit_delete.py` + `backend/tests/integration/test_groups_api.py` + `backend/tests/integration/test_import_export.py`
  - [FR-001~FR-012] [依赖: T8, T11, T13, T14] [出参验证: `pytest backend/tests/integration/` 全部通过（覆盖所有 AC 场景）]

- [X] **T26 [INT]** 前端视觉回归测试：验证 `/watchlist` 页面各组件渲染与设计参考一致
