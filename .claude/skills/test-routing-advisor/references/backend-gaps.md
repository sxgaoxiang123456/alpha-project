# 单后端结构性缺口清单（栈无关能力 → 路由给 `backend-testing`）
...
`backend-testing` skill**，由它读项目的 `package.json` / `pyproject.toml` / `go.mod` /
`Cargo.toml` 等判栈后选该栈对应工具来闭环补测。

> 方法论以 `testing-system-blueprint` skill 为蓝本；缺陷固化复用其指向的 TDD / systematic-debugging
...
|---|---|---|---|
| 真库数据层 / 迁移 / 事务 / 约束 | 流程只 code-review 看（读代码，不跑真库） | ✅ 现成 | → `backend-testing` 按栈选真库 / 容器化测试工具 |
| 鉴权 / 越权（BOLA·BFLA） | 流程**完全没碰**（P0） | 🔧 **需自建** | → `backend-testing` 自建双用户断言（任何栈都无即用方案，见下） |
| 并发 / 竞态 / 限频原子性 | 零覆盖 | ✅ 现成 | → `backend-testing` 按栈选并发压测手段 |
| 韧性 / 重试 / 超时 / 降级 | 流程只 review 看 | ✅ 现成 | → `backend-testing` 按栈选 HTTP / 依赖 mock 库 |
...
- **多栈示例（由 `backend-testing` 按栈选）：** Python 栈常见 `pytest-alembic`（迁移往返）+
...
**自建做法（由 `backend-testing` 按栈落地，逻辑栈无关）：**
...
- **多栈示例（由 `backend-testing` 按栈选）：** Python 栈 `pytest-run-parallel` + `asyncio.gather`；
...
- **多栈示例（由 `backend-testing` 按栈选）：** Python httpx 栈 `respx` / `pytest-httpx`；
...
`backend-testing` 按栈实例化，本 skill 不替它决定。**
