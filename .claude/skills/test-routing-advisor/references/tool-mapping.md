...
> 方法论以 `testing-system-blueprint` skill 为蓝本；本表是其在"feature 收尾路由"环节的落地。
...
|---|---|---|---|
| 单后端 | 真库数据层 / 迁移、并发原子性、韧性 / 降级、对象级越权（BOLA·BFLA）、可选契约 fuzz | → `backend-testing`（按栈解析工具） | ✅ 执行器已建 |
| 单前端 | L0/L1 测试地基、L2 视觉回归、L3 a11y、L4 跨浏览器 + 响应式、L6 前后端契约 mock（设计 token / 硬编码颜色走 lint 门） | → `frontend-testing`（按栈接成熟工具 + 配置 + 翻译视觉契约） | ✅ 执行器已建 |
| 局部前后端 | ① 环境编排（两侧 + 依赖真实同时可复现起来）② 契约真实性（消费者 mock vs 提供者真实对账）③ 接缝粘合（身份透传 / 序列化 / 错误→UI 映射）④ 真实时序 / 实时（条件命中：流式 / 异步才命中） | → `fullstack-slice-testing`（按栈起真栈 + 接缝断言；diff-aware E2E 候选样例 gstack `/qa`，非依赖、无则回退） | ✅ 执行器已建 |
| 完整功能链路 | ① 通路挖掘（从依赖图 / 跨模块契约 / AC 半自动枚举 + 人确认）② 关键性分级 + 选少（P0 安全网，非穷举）③ 全系统编排 + 外部边界 stub（只 stub 外部第三方）④ 异步 / 时间 / 跨通道贯穿（fake clock / 手动触发 job / poll-retry，禁 sleep）⑤ journey 级可追溯 + 安全网定位（首现即回补下层） | → `full-chain-testing`（按栈挖通路 + 全系统编排 + 编排驱动非 UI 跳步；UI 可走段 diff-aware E2E 如 gstack `/qa` 折叠进此处、非依赖、无则回退） | ✅ 执行器已建 |
...
## 单后端 → 路由到 `backend-testing`

单后端的具体工具**不在本 skill 写死**。本 skill 只负责判出"这是单后端、命中了哪几类能力缺口"，
把缺口连同命中理由交给 `backend-testing` skill，由它读 `pyproject.toml` / `package.json` /
...
- **基础单元层：** 用项目所在栈**已在用的那一套**单元测试框架，缺口补测都挂在它下面，不引第二套。
  *（多栈示例：Python 栈常见 `pytest`；JS/TS 栈常见 `vitest` / `jest`；Go 栈用内置 `testing`；
  Rust 栈用内置 `#[test]`——由 `backend-testing` 按栈定，本 skill 不指定。）*
...
- **路由说明：** 本 skill 把命中的能力缺口清单交给 `backend-testing`；任何会写 / 修测试的 agent
  行为都受 5 条护栏约束（见 `self-healing-guardrails.md`）。缺陷固化方法论复用 `testing-system-blueprint`
...
按本 feature 实际触及的东西决定标哪几类缺口，不要无差别全标（具体工具由 `backend-testing` 按栈选）：
...
- **L0/L1 测试地基（🔧 常常为零，第一动作）：** 很多项目 [FE] 出参只写"手测"、没装任何测试运行器，
  地基本身缺失。`frontend-testing` 的**第一动作是立地基**——按栈接 Vitest+RTL（或等价组件测试库）
...
- **路由说明：** 本 skill 把命中的能力缺口清单交给 `frontend-testing`；任何会写 / 修测试的 agent
...
按本 feature 实际触及的东西决定标哪几类缺口，不要无差别全标（具体工具由 `frontend-testing` 按栈接）：
...
## 局部前后端 → 路由到 `fullstack-slice-testing`
...
缺口"，把缺口连同命中理由交给 `fullstack-slice-testing` skill，由它读项目栈（`package.json` /
...
- **路由说明：** 本 skill 把命中的接缝缺口清单交给 `fullstack-slice-testing`；任何会写 / 修测试的
...
按本 feature 实际触及的东西决定标哪几类缺口，不要无差别全标（具体工具由 `fullstack-slice-testing` 按栈接）：
...
缺口"，把缺口连同命中理由交给 `full-chain-testing` skill，由它读项目栈（`package.json` /
...
- **① 通路挖掘（🔧 第一动作，本格独有）：** 需要"**走三源**（静态代码图 + 运行时 trace + spec 契约）
  ...
  藏在依赖图里、要先挖出来——这正是 test-routing-advisor 杀手锏"从依赖图推出 A→B→C 首次贯通"的落地入口。
...
- **② 关键性分级 + 选少（P0 安全网，非穷举）：** 需要"对挖出的通路做关键性分级、只取最高优先级一小撮
  ...
  `testing-system-blueprint` 的 P0–P3。
...
- **路由说明：** 本 skill 把命中的链路缺口清单交给 `full-chain-testing`；任何会写 / 修测试的 agent
...
按本 feature 实际补全了什么链路决定标哪几类缺口，不要无差别全标（具体工具由 `full-chain-testing` 按栈接）：
