# 单前端结构性缺口清单（栈无关能力 → 路由给 `frontend-testing`）
...
任何单栈工具**——判出命中哪些缺口后，连同命中理由一并**路由给 `frontend-testing` skill**，由它读
项目的 `package.json`（含框架 / 测试运行器线索）等判栈后接该栈对应工具来闭环补测。

> 方法论以 `testing-system-blueprint` skill 为蓝本。下文出现的工具名仅为**多栈示例**，非写死答案。
...
- **单前端执行器（`frontend-testing`）"接成熟工具 + 配置 + 把项目视觉契约翻译成断言"** ——
...
|---|---|---|---|
| L0/L1 测试地基 | 流程**完全没碰**——[FE] 出参常只写"手测"、没装测试运行器 | 🔧 第一动作：立地基 | → `frontend-testing` 按栈接组件测试库（Vitest+RTL 等） |
| L2 视觉回归（像素 / 深色 / 颜色契约） | 靠肉眼看截图 | 接工具 + 翻译契约（**人审 L2 基线裁决**） | → `frontend-testing` 按栈接截图快照（Playwright 等） |
| L3 可访问性 a11y（对比度 / ARIA / label） | 零覆盖 | 接工具 | → `frontend-testing` 按栈接 a11y 审计（axe 等） |
| L4 跨浏览器 + 响应式 | 零覆盖 | 接工具（多视口 + 几何断言） | → `frontend-testing` 按栈接多 viewport 运行 |
| L6 前后端契约 mock | 流程没碰，契约易漂移 | 接工具（从规约生成 mock） | → `frontend-testing` 按栈接 MSW + OpenAPI 生成 |
...
只写"手测"、根本没装任何测试运行器——地基本身就是缺的。所以 `frontend-testing` 的**第一动作
是立地基**：按栈装好组件测试库、跑通最小渲染 / 交互用例，之后所有上层缺口补测都挂在它下面，
同一栈内不引第二套框架。

- **多栈示例（由 `frontend-testing` 按栈选）：** JS/TS 栈常见 `vitest` / `jest` + 组件测试库
...
- **多栈示例（由 `frontend-testing` 按栈选）：** Playwright 截图快照等。

### L3. 可访问性 a11y（对比度 / ARIA / label） — 接工具

**能力定义：** 自动审计对比度、ARIA 角色 / 属性、表单 label 关联等可访问性规则。

- **多栈示例（由 `frontend-testing` 按栈选）：** `axe` 系列审计库。

### L4. 跨浏览器 + 响应式 — 接工具

**能力定义：** 在多视口 / 多浏览器下运行，并对布局做几何断言——典型如"任意视口下无横向滚动条"。

- **多栈示例（由 `frontend-testing` 按栈选）：** Playwright 多 project / 多 viewport。
...
- **多栈示例（由 `frontend-testing` 按栈选）：** MSW + 从 OpenAPI 规约生成的 handler。
...
- **多栈示例（由 `frontend-testing` 按栈选）：** `stylelint` 配自定义规则。
...
本 skill 只标命中的能力缺口；**具体工具一律由 `frontend-testing` 按栈实例化，本 skill 不替它决定。**
