# 完整功能链路结构性缺口清单（栈无关能力 → 路由给 `full-chain-testing`）
...
命中哪些缺口后，连同命中理由一并**路由给 `full-chain-testing` skill**，由它读项目栈
（`package.json` / `pyproject.toml` / `docker-compose.yml` / `go.mod` 等）判栈后接该栈对应工具来
闭环补测。

> 方法论以 `testing-system-blueprint` skill 为蓝本。下文出现的工具名（静态代码图 glia / OpenLore、
...
>
> **默认可用的干净实例**：`full-chain-testing` skill 自带一套 **MIT clean-room 自研参考工具**
...
异步消费 / 跨通道流转）。判类的入口正是 test-routing-advisor 的杀手锏——**从依赖图推出某条
...
|---|---|---|---|
| ① 通路挖掘（**三源**枚举端到端通路 + 人确认） | 流程**完全没碰**——通路藏在依赖图里，没人把它列出来；且静态图只挖得动**半张图** | 🔧 第一动作：三源（静态图 + 运行时 trace + spec 契约）半自动枚举 + 人确认 | → `full-chain-testing` 静态图（glia〔非商业 license〕/ OpenLore 式）打底 + 运行时 trace（Pathfinder / Tracetest / OpenTelemetry）补 FE↔BE 桥边与 cron/异步解耦边 + spec 契约（代码未落地时唯一来源） |
| ② 关键性分级 + 选少（P0 安全网，非穷举） | 流程倾向"全测"或"不测" | 给挖出的通路分级、只取 P0 一小撮 | → `full-chain-testing` 按风险分级取少数 P0 通路（蓝本 P0–P3） |
| ③ 全系统编排 + 外部边界 stub | 各 feature 各自 mock，整系统从没拼起来 | 起整系统，只 stub 最外层第三方 | → `full-chain-testing` 起真整栈（docker-compose / 进程内组装）+ 仅 stub 外部第三方（WireMock 等） |
| ④ 异步 / 时间 / 跨通道贯穿 | 定时 / 异步 / 跨通道跳步零端到端覆盖 | fake clock + 手动触发 job + poll-retry，**禁 sleep** | → `full-chain-testing` 接 fake clock / 手动触发 + 轮询断言（Playwright `page.clock` 等） |
| ⑤ journey 级可追溯 + 安全网定位 | 失败只指向某个 feature，看不出整条旅程 | 把断言挂到 journey 步骤 + 首现即回补下层 | → `full-chain-testing` journey↔AC 可追溯 + "首次在此发现=下层漏测"回补提示 |
...
test-routing-advisor 杀手锏"从依赖图推出 A→B→C 首次贯通"的落地入口——**那条刚贯通的链路，就是通路
挖掘的起点**。
...
- **多栈示例（由 `full-chain-testing` 按栈选，均为可选实例、非依赖）：** 静态图用 glia〔非商业
...
反而拖垮信号。分级标准遵循 `testing-system-blueprint` 的 P0–P3 风险分级。

- **多栈示例（由 `full-chain-testing` 按栈选）：** 按 AC 的"必须成立"程度 + 旅程触达用户面排序，取
...
- **多栈示例（由 `full-chain-testing` 按栈选）：** `docker-compose` 起整系统；进程内组装多 feature 的
...
- **多栈示例（由 `full-chain-testing` 按栈选）：** Playwright `page.clock` 或后端 fake clock 快进时间；
...
- **多栈示例（由 `full-chain-testing` 按栈选）：** journey 步骤↔AC 可追溯矩阵；失败时输出"这是
...
`full-chain-testing` 按栈实例化（含 gstack `/qa` 这类有则优先、无则回退的非硬依赖），本 skill 不替
它决定。**
