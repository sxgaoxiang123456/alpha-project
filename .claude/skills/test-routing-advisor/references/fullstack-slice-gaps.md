# 局部前后端结构性缺口清单（栈无关能力 → 路由给 `fullstack-slice-testing`）
...
不写死任何单栈工具**——判出命中哪些缺口后，连同命中理由一并**路由给 `fullstack-slice-testing`
skill**，由它读项目栈（`package.json` / `pyproject.toml` / `docker-compose.yml` / `go.mod` 等）
判栈后接该栈对应工具来闭环补测。

> 方法论以 `testing-system-blueprint` skill 为蓝本。下文出现的工具名（docker-compose / Playwright /
...
- **单后端执行器（`backend-testing`）"自己写测试代码"** —— 越权双用户断言、并发不变量等多半要手写。
- **单前端执行器（`frontend-testing`）"接成熟工具 + 配置 + 翻译视觉契约"** —— 工具现成，活儿在配置。
- **局部前后端执行器（`fullstack-slice-testing`）"先把真栈起起来，再对账"** —— 断言本身不难写，
...
|---|---|---|---|
| ① 环境编排（两侧 + 依赖真实同起） | 流程**完全没碰**——两侧各自 mock 跑绿，真栈从没拼起来 | 🔧 第一动作：起真栈 | → `fullstack-slice-testing` 按栈起（docker-compose / 进程内组装 + testcontainers / gstack） |
| ② 契约真实性（mock vs 真提供者对账） | 单前端用消费者 mock 跑绿，没对过真后端形状 | 把 mock 换真提供者对账 | → `fullstack-slice-testing` 回放真响应校验 mock / 双向 diff（Pact / OpenAPI diff） |
| ③ 接缝粘合（身份透传 / 序列化 / 错误→UI） | 单侧测试都看不到的粘合点 | 带真鉴权头打通 + 断言 UI 错误态 | → `fullstack-slice-testing` 跨进程驱动 + UI 状态断言 |
| ④ 真实时序 / 实时（条件命中） | 流式 / 异步路径零覆盖 | 接工具断言增量到达时序 | → `fullstack-slice-testing` 接 SSE / WebSocket / 轮询断言（**仅流式 / 异步才标**） |
...
- **多栈示例（由 `fullstack-slice-testing` 按栈选）：** `docker-compose` 起整栈；进程内 ASGI/WSGI
...
- **多栈示例（由 `fullstack-slice-testing` 按栈选）：** 用真后端响应回放校验前端的 MSW handler；
...
- **多栈示例（由 `fullstack-slice-testing` 按栈选）：** 带真实鉴权头打通整条请求路径，断言受保护资源
...
- **多栈示例（由 `fullstack-slice-testing` 按栈选）：** SSE / WebSocket 增量与时序断言；轮询到达
  时序断言。**按栈定。**
...
不进本格。本 skill 只标命中的接缝缺口；**具体工具一律由 `fullstack-slice-testing` 按栈实例化（含
