...
class GrokSearchProvider(BaseSearchProvider):
    ...
    async def _execute_non_stream(self, headers: dict, payload: dict, ctx=None) -> str:
        """执行非流式 HTTP 请求作为 fallback"""
            ...
            async for attempt in AsyncRetrying(
                ...
                with attempt:
                    ...
                    if choices and len(choices) > 0:
                        ...
                        content = message.get("content", "")
                        await log_info(ctx, f"non-stream content: {content[:200]}", config.debug_enabled)
                        return content
                    return ""

    async def _execute_stream_with_retry(self, headers: dict, payload: dict, ctx=None) -> str:
        """执行带重试机制的流式 HTTP 请求，空内容时 fallback 到非流式"""
        timeout = httpx.Timeout(connect=6.0, read=120.0, write=10.0, pool=None)
            ...
            async for attempt in AsyncRetrying(
                ...
                with attempt:
                    async with client.stream(
                        ...
                        if content:
                            return content
                        # 流式返回空，尝试非流式 fallback
                        await log_info(ctx, "Stream returned empty, trying non-stream fallback", config.debug_enabled)
                        return await self._execute_non_stream(headers, payload, ctx)
